...
class MaintPlanVC: UITableViewController {
    ...
    @objc func refreshData() {
        ...
        let url = "{\"txcode\":\"\(CommonData.TXCODE_MAINT_PLAN)\",\"imei\":\"\(CommonData.TERMINAL_IDENTIFICATION)\",\"maintId\":\"\(CommonData.MAINT_ID)\",\"clientId\":\"\(CommonData.CLIENT_ID)\",\"text\":\"\(CommonData.MAINT_PLAN_SEARCH)\"}"
        print(url)
                    ...
                    if rescode == "0" {
                        if(json["data"].count>0){
                            ...
                            for i in 0...(json["data"].count - 1) {
                                ...
                                resultData["longitude"] = json["data"][i]["longitude"].string!
                                resultData["latitude"] = json["data"][i]["latitude"].string!
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:MaintPlanTableViewCell = tableView.dequeueReusableCell(withIdentifier: "maintPlanCell") as! MaintPlanTableViewCell
        if dataArray.count>0 {
        let item = self.dataArray[indexPath.row]
        cell.orderId.text = item["formcode"]
        cell.dateTime.text = item["plandate"]
        cell.codeId.text = item["liftcode"]
        cell.address.text = item["liftaddress"]
        cell.address.sizeToFit()
        }
        return cell
    }
    override  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let startMaintVC = StartMaintViewController()
        startMaintVC.title = CommonData.TITLE_NAVI_RESUCE_MAP
        startMaintVC.dataDetail = self.dataArray[indexPath.row]
        self.navigationController?.pushViewController(startMaintVC, animated: true)
    }
    func drawNullView(){
