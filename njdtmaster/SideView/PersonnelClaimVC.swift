...
class PersonnelClaimVC: UIViewController ,UITableViewDataSource,UITableViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        //设置搜索菜单
//        searchIV.frame = CGRect(x: 5, y: 67 + CommonData.ADD_ADADTATION_MAX_HEIGHT, width: 32, height: 32)
//        self.view.addSubview(searchIV)
//        let searchTF = UITextField()
//        searchTF.frame = CGRect(x: 37, y: 67 + CommonData.ADD_ADADTATION_MAX_HEIGHT, width: CommonData.ADMIN_SRCEEN_WIDTH - 40, height: 32)
//        searchTF.placeholder = CommonData.PLACEHOLDER_PERSONNEL_CLAIM_SEARCH
//        searchTF.borderStyle = UITextBorderStyle.line
//        self.view.addSubview(searchTF)
    ...
    @objc func refreshData() {
        ...
        Alamofire.request(CommonData.CONSTANT_PATH_POST_96333, method: .post, parameters: ["data":"{\"txcode\":\"\(CommonData.TXCODE_ALL_CLIENT)\",\"imei\":\"\(CommonData.TERMINAL_IDENTIFICATION)\",\"maintId\":\"\(CommonData.MAINT_ID)\"}"], encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
    ...
    @objc func submitClaim (sender: UIButton){
        Alamofire.request(CommonData.CONSTANT_PATH_POST_96333, method: .post, parameters: ["data":"{\"claim\":\"\(CommonData.PERSONNEL_CLAIM_ARRAY)\",\"logoff\":\"\",\"txcode\":\"\(CommonData.TXCODE_CLAIM)\",\"maintId\":\"\(CommonData.MAINT_ID)\",\"imei\":\"\(CommonData.TERMINAL_IDENTIFICATION)\"}"], encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
            switch response.result.isSuccess {
            case true:
                if let value = response.result.value {
                    let json = JSON(value)
...
extension PersonnelClaimVC: CheckboxDelegate {
    func didSelect(_ checkbox1: CCheckbox) {
        print("当前选中编号：\(checkbox1.tag)")
        CommonData.PERSONNEL_CLAIM_ARRAY.append(self.dataArray[checkbox1.tag]["id_hidden"]!)
        for value in CommonData.PERSONNEL_CLAIM_ARRAY {
            print(value)
        }
    }
    ...
    func didDeselect(_ checkbox1: CCheckbox) {
        ...
        if CommonData.PERSONNEL_CLAIM_ARRAY.count > 0 {
            CommonData.PERSONNEL_CLAIM_ARRAY.remove(at: CommonData.PERSONNEL_CLAIM_ARRAY.index(of: self.dataArray[checkbox1.tag]["id_hidden"]!)!)
        }else{
            print("待认领人员数组为空！")
        }
    }
